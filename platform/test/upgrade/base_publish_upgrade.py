...
import shutil
import unittest
from pulp.server.upgrade.main import DB_UPGRADE_CALLS, FILES_UPGRADE_CALLS
...
class BasePublishUpgradeTests(unittest.TestCase):
    ...
    def setUp(self):
        super(BasePublishUpgradeTests, self).setUp()
        ...
        self._perform_fs_upgrade(self.v1_test_db.database, self.v2_test_db.database)
    ...
    def _perform_fs_upgrade(selfself, v1_database, v2_database):
        for (fs_call, description) in FILES_UPGRADE_CALLS:
            fs_call(v1_database, v2_database)

    def tearDown(self):
        super(BasePublishUpgradeTests, self).tearDown()
